...
The same verified sample experience has also completed Mode B deployment-test validation as a target-project embedded host module. This roadmap does not re-prove either sample validation; it preserves them as the MVP baseline for a reusable integration package.
