...
test("CLI can analyze, plan, generate, and verify an image-agent-web-like target", () => {
  ...
  const embeddedHostVerifyOutput = runExpectFailure(["verify", missingGenerated, "--mode", "embedded-adapter", "--runtime-url", "http://127.0.0.1:3978", "--simulate", "--strict"]);
  assert.match(embeddedHostVerifyOutput, /embedded:host-runtime-url/);
  const embeddedHostVerifyReport = JSON.parse(fs.readFileSync(path.join(missingGenerated, "verification_report.json"), "utf8"));
  assert.equal(embeddedHostVerifyReport.status, "warn");
  ...
  assert.ok(embeddedHostVerifyReport.checks.some((item) => item.name === "embedded:host-runtime-url" && item.status === "warn"));
