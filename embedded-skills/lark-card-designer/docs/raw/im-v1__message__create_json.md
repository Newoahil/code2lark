...
### 文本 text
**注意**：如需发送 Markdown 内容，推荐使用富文本 `post` 消息中的 `md` 标签，支持 CommonMark 0.31 + GFM 语法，包括表格、任务列表、删除线、自动链接等能力。除此之外，飞书还额外支持 @ 用户、@ 所有人和图片扩展语法。`text` 类型仅适用于普通文本和部分轻量格式。
...
### 富文本 post
**注意**：发送 Markdown 内容，推荐使用富文本 `post` 消息中的 `md` 标签，支持 CommonMark 0.31 + GFM 语法，包括表格、任务列表、删除线、自动链接等能力。除此之外，飞书还额外支持 @ 用户、@ 所有人和图片扩展语法。
...
- **md：Markdown 标签**
- 支持 CommonMark 0.31 + GFM 常用语法，包括表格、任务列表、删除线、自动链接等能力。除此之外，飞书还额外支持 @ 用户、@ 所有人和图片等扩展语法。
- 部分样式需要升级到最新客户端版本查看。warning
**注意**：
- `md` 标签会独占一个或多个段落，不能与其他标签在同一行。
- `md` 标签在旧版本消息结构`content`字段中会被兼容适配，如[获取消息内容](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/get)时将不再包含此标签，会根据 `md` 中的内容转换为其他相匹配的标签。例如引用，有序、无序列表，表格等在获取消息内容时，会简化为文本标签（text）进行输出。
- GFM 支持但飞书不支持的语法：Setext 标题，缩进代码块，任意 raw HTML，邮箱自动链接。
...
语法 | 示例 | 说明
---|---|---
@ 用户 | `<at user_id="ou_xxx">User</at>` | 飞书扩展语法。支持 @ 单个用户或所有人。<br>- @ 单个用户时，需要在 `user_id` 内传入实际用户的真实 ID。传入的值可以是用户的 user_id、open_id、union_id。各类 ID 获取方式参见[如何获取 User ID、Open ID 和 Union ID](https://open.feishu.cn/document/home/user-identity-introduction/open-id)。<br>- 如需 @ 所有人，需要将 `user_id` 取值为 `all`，即 `<at user_id="all"></at>`。
图片 | `![alt](img_v3_xxx)` | 飞书扩展语法。支持在 Markdown 中插入图片。<br>图片语法中的链接地址只需要传图片 Key 字符串，例如 `img_v3_xxx`。图片 Key 可通过[上传图片](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/image/create)接口获取。
普通文本 / 段落 / 换行 | `hello\nworld` | 支持普通文本、段落和换行。<br>**注意**：在 Markdown 中，单个 `\n` 通常会被解析为软换行。如需分隔不同块级元素，建议使用 `\n\n`。
ATX 标题 | `# h1\n## h2\n### h3` | 支持 ATX 标题语法，即使用 `#` 表示标题层级，支持 `#` 到 `######`。<br>**注意**：暂不支持 Setext 标题语法，例如 `Heading\n---`。如需设置标题，建议使用 ATX 标题。
加粗 | `**加粗文本**`<br>`__加粗文本__` | 支持使用 `**` 或 `__`包裹文本设置加粗。
斜体 | `*斜体文本*`<br>`_斜体文本_` | 支持使用 `*` 或 `_` 包裹文本设置斜体。<br>斜体可以与加粗组合使用，例如：`***加粗+斜体***`。
删除线 | `~~删除线文本~~` | 支持 GFM 删除线语法，使用 `~~` 包裹文本设置删除线。
下划线 | `<u>下划线文本</u>` | 支持 GFM 下划线语法，使用 `<u>` `</u>` 包裹文本设置下划线。
行内代码 | \`code\` | 支持使用反引号包裹行内代码。
代码块 | \`\`\`typescript\nconsole.log("hello")\n\`\`\` | 支持围栏代码块。代码块内容首尾需要使用 \`\`\` 包裹，首部 \`\`\` 后可以指定代码语言。<br>支持的语言类型包括 PYTHON、C、CPP、GO、JAVA、KOTLIN、SWIFT、PHP、RUBY、RUST、JAVASCRIPT、TYPESCRIPT、BASH、SHELL、SQL、JSON、XML、YAML、HTML、THRIFT 等，不区分大小写。<br>**注意**：暂不支持缩进代码块，建议使用围栏代码块。
引用 | `> quote` | 支持引用块语法。`>` 符与后续内容之间建议保留一个空格。<br>**注意**：引用块后面如需接其他块级元素，**必须**使用 `\n\n` 分隔。
有序列表 | `1. item1\n2. item2` | 支持有序列表。<br>Markdown 配置说明：<br>- 每个编号的 `.` 符与后续内容之间要有一个空格。<br>- 每一列独立一行。如示例所示，可使用 `\n` 换行符换行。<br>- 支持嵌套多层级。<br>- 可以与无序列表混合使用。
无序列表 | `- item1\n- item2` | 支持无序列表。<br>Markdown 配置说明：<br>- 每列的 `-` 符与后续内容之间要有一个空格。<br>- 每一列独立一行。如示例所示，可使用 `\n` 换行符换行。<br>- 支持嵌套多层级。<br>- 可以与有序列表混合使用。
任务列表 | `- [ ] todo\n- [x] done` | 支持 GFM 任务列表语法。<br>**注意**：任务列表仅用于展示任务状态，不等同于飞书任务。
表格 | `| 名称 | 链接 |\n| --- | --- |\n| 飞书 | https://www.feishu.cn |` | 支持 GFM 表格语法。表格不支持嵌套，表格内不支持图片。<br>**注意**：表格前后**必须**使用 `\n\n` 与其他块级元素分隔。如果只使用单个 `\n`，后续内容可能被解析为表格的续行。
分割线 | `\n --- \n` | 支持 Markdown 分割线语法。
超链接 | `[飞书](https://www.feishu.cn)` | 支持 Markdown 超链接语法。`[]` 用来设置超链接的文本内容，`()` 用来设置超链接地址。<br>**注意**：请确保链接地址合法。
URL 自动链接 | `<https://www.feishu.cn>` | 支持 URL 自动链接。`<url>` 形式均可被识别为链接。<br>**注意**：暂不支持邮箱自动链接，例如 `<user@feishu.cn>`。
脚注 | `这里有一个脚注引用[^1]\n\n[^1]: 脚注内容` | 支持脚注语法。<br>**注意**：脚注支持展示，但暂不支持点击定位。
